...
Arduino 版（`.ino` 冒頭）も同じ `STEP` / `DIRECTION` を持ち、加えて次を持つ。

| 定数 | 既定値 | 内容 |
|------|-------|------|
| `COUNTS_PER_DETENT` | 4 | 1カチあたりのクアドラチャ・カウント数。**何回か回さないと動かない／取りこぼす個体は 2 や 1 に下げる** |

- 1カチで2ずつ動く個体（ハーフステップ型）は `STEP` を半分にする。

### Arduino版のデコード方式について（無反応だった不具合の修正）

初版は厳密な状態機械（Ben Buxton 方式）で復号していたが、これは「A相とB相が1つずつ
順番に変化する」完全な信号列を前提にしており、接点バウンス等で状態が飛ぶと
カウントを出さずにリセットし続ける。実機のログでは `A=1 B=1` → `A=0 B=0` のように
両相が同時に変化する遷移があり、**この方式では出力が一切出なかった**。

現在は、前回ABと今回ABの組み合わせから増減を引くテーブル方式（QEM）に変更している。
不正な遷移は 0 として捨てるだけで停止しないため、信号が多少飛んでも動き続ける。
端数のカウントは次回に持ち越すので、ゆっくり回しても取りこぼさない。
